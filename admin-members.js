...
<head>
	...
	<style nonce="${nonce}">
		...
		* { box-sizing:border-box } body { margin:0; min-width:280px; background:#f4f7fb } button,input,select { font:inherit }
		...
		.quick-days,.form-actions,.secret-actions,.customer-actions { display:flex; flex-wrap:wrap; gap:.55rem; align-items:center } .customer-actions { max-width:360px }
		...
		.table-wrap { overflow-x:auto } table { width:100%; border-collapse:collapse; min-width:1280px }
...
<body>
	<main class="page">
		...
		<section id="package-panel" class="panel edit-panel" aria-labelledby="package-title" hidden>
	<div class="edit-heading"><div><h2 id="package-title">&#22871;&#39184;&#31649;&#29702;</h2><div id="package-customer-id" class="edit-customer-id"></div></div><button id="package-close" class="secondary" type="button">Close</button></div>
	<div class="secret-grid package-readonly">
		<div class="secret-label">Expires</div><div id="package-expires" class="secret-value"></div>
		<div class="secret-label">Remaining</div><div id="package-remaining" class="secret-value"></div>
		<div class="secret-label">Quota</div><div id="package-quota" class="secret-value"></div>
		<div class="secret-label">Used</div><div id="package-used" class="secret-value"></div>
		<div class="secret-label">Traffic remaining</div><div id="package-traffic-remaining" class="secret-value"></div>
		<div class="secret-label">Usage updated</div><div id="package-usage-updated" class="secret-value"></div>
		<div class="secret-label">Settled through</div><div id="package-settled-through" class="secret-value"></div>
		<div class="secret-label">Enabled</div><div id="package-enabled" class="secret-value"></div>
		<div class="secret-label">Traffic eligibility</div><div id="package-eligibility" class="secret-value"></div>
	</div>
	<div class="form-actions">
		<button id="package-renew" class="secondary" type="button">&#32493;&#36153;30&#22825;</button><button id="package-renew-custom" class="secondary" type="button">&#33258;&#23450;&#20041;&#32493;&#36153;</button>
		<button id="package-add-quota" class="secondary" type="button">&#22686;&#21152;&#27969;&#37327;</button>
		<button id="package-combined" type="button">&#32493;&#36153;&#65291;&#22686;&#21152;&#27969;&#37327;</button>
		<button id="package-set-quota" class="secondary" type="button">&#35774;&#32622;&#24635;&#39069;</button>
		<button id="package-unlimited" class="secondary" type="button">&#19981;&#38480;&#27969;&#37327;</button>
		<button id="package-set-used" class="secondary" type="button">&#26657;&#27491;&#24050;&#29992;</button>
		<button id="package-reset-used" class="secondary" type="button">&#28165;&#386零;&#24050;&#29992;</button>
		<button id="package-toggle" class="danger" type="button">&#20572;&#29992;</button>
	</div>
</section><section class="panel" aria-labelledby="stats-title">
	...
	(() => {
		...
		const elements = {
			...
			packagePanel: document.getElementById('package-panel'), packageCustomerId: document.getElementById('package-customer-id'), packageExpires: document.getElementById('package-expires'), packageRemaining: document.getElementById('package-remaining'), packageQuota: document.getElementById('package-quota'), packageUsed: document.getElementById('package-used'), packageTrafficRemaining: document.getElementById('package-traffic-remaining'), packageUsageUpdated: document.getElementById('package-usage-updated'), packageSettledThrough: document.getElementById('package-settled-through'), packageEnabled: document.getElementById('package-enabled'), packageEligibility: document.getElementById('package-eligibility'), packageRenew: document.getElementById('package-renew'), packageRenewCustom: document.getElementById('package-renew-custom'), packageAddQuota: document.getElementById('package-add-quota'), packageCombined: document.getElementById('package-combined'), packageSetQuota: document.getElementById('package-set-quota'), packageUnlimited: document.getElementById('package-unlimited'), packageSetUsed: document.getElementById('package-set-used'), packageResetUsed: document.getElementById('package-reset-used'), packageToggle: document.getElementById('package-toggle'), packageClose: document.getElementById('package-close'),
		...
		function createActionButton(label, className, disabled, handler) {
			...
			elements.packageCustomerId.textContent = '';
			elements.packagePanel.hidden = true;
		}
		function renderPackagePanel() {
			const customer = packageCustomerId ? customersById.get(packageCustomerId) : null;
			if (!customer) { elements.packagePanel.hidden = true; return; }
			elements.packagePanel.hidden = false;
			elements.packageCustomerId.textContent = customer.customerId;
			elements.packageExpires.textContent = formatLocalTime(customer.expiresAt);
			elements.packageRemaining.textContent = formatRemaining(customer.expiresAt);
			elements.packageQuota.textContent = customer.trafficInvalid ? 'Traffic data invalid' : customer.unlimitedTraffic ? 'Unlimited' : formatTrafficBytes(customer.quotaBytes);
			elements.packageUsed.textContent = customer.trafficInvalid ? 'Traffic data invalid' : formatTrafficBytes(customer.settledUsedBytes);
			elements.packageTrafficRemaining.textContent = customer.trafficInvalid ? 'Traffic data invalid' : customer.unlimitedTraffic ? 'Unlimited' : formatTrafficBytes(customer.remainingBytes);
			elements.packageUsageUpdated.textContent = customer.trafficInvalid ? 'Time invalid' : formatUsageUpdatedAt(customer.usageUpdatedAt);
			elements.packageSettledThrough.textContent = customer.trafficInvalid ? 'Time invalid' : formatUsageUpdatedAt(customer.usageSettledThrough);
			elements.packageEnabled.textContent = customer.enabled ? 'Enabled' : 'Disabled';
			elements.packageEligibility.textContent = customer.trafficInvalid ? 'Pending review' : customer.trafficEligible ? 'Eligible' : 'Quota exhausted';
			const busy = customerOperations.has(customer.customerId), active = customer.state === 'active' && !customer.timeInvalid;
			[ elements.packageRenew, elements.packageRenewCustom, elements.packageAddQuota, elements.packageCombined, elements.packageSetQuota, elements.packageUnlimited, elements.packageSetUsed, elements.packageResetUsed, elements.packageToggle ].forEach((button) => { button.disabled = busy || !active || (customer.trafficInvalid && button !== elements.packageRenew && button !== elements.packageRenewCustom && button !== elements.packageCombined && button !== elements.packageToggle); });
			elements.packageToggle.textContent = customer.enabled ? '\u505c\u7528' : '\u542f\u7528';
		}
		function openPackagePanel(customerId) {
			if (!customersById.has(customerId)) return;
			packageCustomerId = customerId;
			renderPackagePanel();
			elements.packagePanel.scrollIntoView({ block:'nearest' });
		}
		async function runPackageCombined() {
			const customer = packageCustomerId ? customersById.get(packageCustomerId) : null;
			if (!customer || customer.state !== 'active' || customer.timeInvalid || customer.trafficInvalid || customerOperations.has(customer.customerId)) return;
			const dayValue = window.prompt('Renew days (1-3650)', '30'); if (dayValue === null) return;
			const quotaValue = window.prompt('Additional quota GiB (1-1000000)', '100'); if (quotaValue === null) return;
			let durationDays, quotaGiB;
			try { durationDays = validateDurationDays(dayValue); quotaGiB = validateQuotaGiB(quotaValue); } catch (error) { showMessage(error.message, 'error'); return; }
			if (!window.confirm('This updates validity and traffic quota together. Continue?')) return;
			const customerId = customer.customerId;
			customerOperations.set(customerId, 'package'); clearMessage(); renderAll();
			try {
				const result = await mutationRequest('/admin/api/customers/' + encodeURIComponent(customerId) + '/package', 'POST', { durationDays, addQuotaGiB:quotaGiB, expectedRevision:customer.revision, expectedUsageRevision:customer.usageRevision }, 200, validateCustomerEnvelope);
				upsertCustomer(result.customer); showMessage('Package updated', 'info');
			} catch (error) { if (error.message !== 'redirecting') showMessage(safeMutationErrorMessage(error), 'error'); }
			finally { customerOperations.delete(customerId); renderAll(); }
		}
		async function runPackageRenewCustom() { const customer = packageCustomerId ? customersById.get(packageCustomerId) : null; if (customer) await renewCustomerCustom(customer.customerId); }
		async function runPackageRenew() { const customer = packageCustomerId ? customersById.get(packageCustomerId) : null; if (customer) await renewCustomer(customer.customerId, 30); }
		async function runPackageAddQuota() { const customer = packageCustomerId ? customersById.get(packageCustomerId) : null; if (customer) await addCustomerQuota(customer.customerId); }
		async function runPackageSetQuota() { const customer = packageCustomerId ? customersById.get(packageCustomerId) : null; if (customer) await setCustomerQuota(customer.customerId); }
		async function runPackageUnlimited() { const customer = packageCustomerId ? customersById.get(packageCustomerId) : null; if (customer) await setCustomerUnlimited(customer.customerId); }
		async function runPackageSetUsed() { const customer = packageCustomerId ? customersById.get(packageCustomerId) : null; if (customer) await setCustomerUsedTraffic(customer.customerId); }
		async function runPackageResetUsed() { const customer = packageCustomerId ? customersById.get(packageCustomerId) : null; if (customer) await resetCustomerUsage(customer.customerId); }
		async function runPackageToggle() { const customer = packageCustomerId ? customersById.get(packageCustomerId) : null; if (customer) await toggleCustomer(customer.customerId, !customer.enabled); }
function createActionsCell(customer) {
			...
			actions.appendChild(createActionButton('\u7f16\u8f91\u8d44\u6599', 'secondary', busy, () => openEditPanel(customer.customerId)));
			actions.appendChild(createActionButton('\u5957\u9910\u7ba1\u7406', 'secondary', busy, () => openPackagePanel(customer.customerId)));
		...
		function renderAll() {
			renderStats(); renderMigrationNotice(); renderCustomers(); updateLoadMoreButton(); renderPackagePanel();
		...
		elements.packageClose.addEventListener('click', closePackagePanel);
		elements.packageRenew.addEventListener('click', () => { void runPackageRenew(); });
		elements.packageRenewCustom.addEventListener('click', () => { void runPackageRenewCustom(); });
		elements.packageAddQuota.addEventListener('click', () => { void runPackageAddQuota(); });
		elements.packageCombined.addEventListener('click', () => { void runPackageCombined(); });
		elements.packageSetQuota.addEventListener('click', () => { void runPackageSetQuota(); });
		elements.packageUnlimited.addEventListener('click', () => { void runPackageUnlimited(); });
		elements.packageSetUsed.addEventListener('click', () => { void runPackageSetUsed(); });
		elements.packageResetUsed.addEventListener('click', () => { void runPackageResetUsed(); });
		elements.packageToggle.addEventListener('click', () => { void runPackageToggle(); });
